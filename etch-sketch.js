...
function drawGrid(gridSize) {
  ...
  for (let i = 0; i < gridSize; i++) {
    ...
    for (let j = 0; j < gridSize; j++) {
      ...
      innerDiv.style.height = (960 / gridSize).toString() + "px";
      innerDiv.style.width = (960 / gridSize).toString() + "px";
...
container.addEventListener("mouseover", (event) => {
  ...
  if (target.className === "inner" && isMouseDown === true) {
    // hmmm??? TODO:
...
newGridBtn.addEventListener("click", () => {
  const size = parseInt(
    prompt("Enter a new grid size, from 1 to 64 (inclusive) for a new grid")
  );
